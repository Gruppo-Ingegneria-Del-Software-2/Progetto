var cal = {
  ...
  //    document.getElementById("name-container").innerHTML = this.response;
    };
    xhr.send(data);
  },
  ...
  //  data.append('name', document.getElementById('evt-name').value);
    ...
    xhr.onload = function(){
      if (this.response=="OK") {
        ...
        alert("Errore del salvataggio dell'evento");
      }
    };
    xhr.send(data);
    return false;
    ...
    if (confirm("Elimina evento?")) {
      ...
      xhr.onload = function(){
        if (this.response=="OK") {
          ...
          alert("Errore della cancellazione dell'evento");
        }
      };
      xhr.send(data);
    }
