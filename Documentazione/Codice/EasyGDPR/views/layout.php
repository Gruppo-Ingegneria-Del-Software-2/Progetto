  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    event_notifications = [
      ...
      for (i = 0; i < event_notifications.length; i++) { 
        ...
        // Massima urgenza
        if ( event.urgency == "3" && event.type != "1" ){
          if (now < start_date) {
              console.log("Non finisce oggi");
          ...
          //   //  console.log("Non finisce oggi");
          //   //  tmp_end_counter = moment().add(1, 'days')
          //   while( now < tmp_start_time ){
          //       event.notify_time.push(tmp_start_time.format('HH:mm'))
          //       tmp_start_time = tmp_start_time.add(-1, 'hours')
          //     }
          //   } 
          }
      ...
      function setNotifications(){
        ...
        var end_time = moment(event['end_time'],'HH:mm');
        var start_time = moment(event['start_time'],'HH:mm');
         //moment(event['start_time'],'hour').fromNow();
        ...
        for (j = 0; j < event_notifications.length; j++) { 
          ...
          if(event_notifications[j].notify_time.includes(now_time)){
            if (event_notifications[j].type == "1") {
              ...
              var duration = moment.duration(moment(event_notifications[j].start_date,'YYYY-MM-DD HH:mm').diff(moment()));
            }
            ...
            if(!already_showed_notifications.includes(notification_id)){
              ...
              if ( event_notifications[j].type == '1' && event_notifications[j].urgency == '3') {
                if ( hours < 1 ) {
                  new Noty({
                text: "Mancano "+minutes+" minuti alla scadenza '"+event_notifications[j].name+"'",
                layout   : 'topRight',
                theme    : 'relax',
                type : noty_type,
                ...
                callbacks: {
                  ...
                  close: 'animated fadeOutRight'
                    ...
                    new Noty({
                    text: "Mancano "+hours+" ore alla scadenza '"+event_notifications[j].name+"'",
                    layout   : 'topRight',
                    theme    : 'relax',
                    type : noty_type,
                    closeWith: ['click', 'button'],
                ...
                if ( hours < 1){//timestimestart.asHtimestart.asHtart.asH
                  new Noty({
                text: "Mancano "+minutes+" minuti all'inizio dell'evento '"+event_notifications[j].name+"'",
                layout   : 'topRight',
                theme    : 'relax',
                type : noty_type,
                closeWith: ['click', 'button'],
                ...
                new Noty({
                text: "Mancano "+hours+" ore all'inizio dell'evento '"+event_notifications[j].name+"'",
                layout   : 'topRight',
                theme    : 'relax',
                type : noty_type,
                closeWith: ['click', 'button'],
